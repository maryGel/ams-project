...
export default function RefUom({useProps, openTab,}) {
  const {uomData, loading, error} = useRefUom(useProps)
        ...
        <ThemeProvider theme={customTheme}>
          <div className='w-auto p-4 h-full min-h-[600px] bg-white rounded-xl shadow-lg'>
            ...
            <div className='flex items-center justify-between mt-6 mb-4'>
              <h1 className='text-xl font-semibold text-gray-800'>UOM List</h1>
            ...
            >
              {/* ----------------------- TABLE STARTS HERE --------------------------- */}
              <table
                className="border-collapse w-300"
                ...
                <tbody>
                  {(editMode ? temporaryData : (rows || []).slice(0, 5)).map((row) => (
                    ...
                    >
                      ...
                      {/* Unit column */}
                      <td className="p-2 border-r border-gray-200 pl-2text-sm bg-gray-50" style={tbodyStyle('Unit')}>
                        {editMode ? (
                          <input
                            type="text"
                            value={row.Unit ?? ''}
