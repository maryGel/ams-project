...
function MvAAForm({
  ...
  const hideToast = useCallback(() => {
    ...
    const getItemsByAANo = (AAFNo) => {
        return assetAccDetails?.filter(detail => detail.AAFNo === AAFNo) || [];
    };
    const getAppLogByAANo = (AAFNo) => {
        return approvalLogs?.filter(log => (log.TRNO === AAFNo) && log.Module === 'Asset Accountability') || [];
    }

        // Sort the filteredAA array by date (latest first)
    const sortedFilteredAA = useMemo(() => {
        if (!filteredAA) return [];
        
        return [...filteredAA].sort((a, b) => {
  ...
  return (
    <>
      ...
      <BulkActionDialog
        ...
        itemCount={selectedAA.length}
        itemName="Asset Accountability"
        loading={bulkLoading}
        remarks={bulkRemarks}
        onRemarksChange={setBulkRemarks}
      />

      {selectionMode && selectedAA.length > 0 && (
        <SelectionModeHeader
          selectedCount={selectedAA.length}
          ...
          return (
            <div key={header?.ID} 
              className={clsx(
                ...
                (selectedAA.includes(header.AAFNo) && eligibleForSelection) && "bg-slate-200 border-blue-500",
              ...
              onClick={() => {
                ...
                if (selectionMode && eligibleForSelection) {
                  if (selectedAA.includes(header.AAFNo)) {
                    setSelectedAA(prev => prev.filter(j => j !== header.AAFNo));
                  } else {
                    setSelectedAA(prev => [...prev, header.AAFNo]);
                  }
                }
              }}
          ...
          <div className='fixed inset-0 bg-black bg-opacity-50 flex items-end z-[99999] animate-fade-in'>
            ...
            >
              <div className='sticky top-0 px-6 py-4 bg-white border-b rounded-t-2xl'>
                <div className='flex items-center justify-between'>
                  <div>
                    <h3 className='text-lg font-semibold text-gray-900'>Asset Accountability Approval</h3>
              ...
              <div className='p-6'>
                <div className='mb-4'>
                  ...
                  <TextareaAutosize
                    ...
                    disabled={processingAA === header.AAFNo}
                    autoFocus
                  />
                </div>
              ...
              >
                ...
                <div className='p-6'>
                  ...
                  <div className="grid grid-cols-2 gap-2 text-sm">
                    ...
                    <div>
                      <div className="text-gray-400">Doc NO:</div>
                    ...
                    {selectedDetails.Supplier &&
                      <>
                        <div>
                          ...
                          <div >{selectedDetails.Supplier}</div>
                        </div>
                      </>
                    }   
                    {selectedDetails.unitcost &&
                    <>
                      <div>
                        <div className="text-sm text-gray-400">Unit Cost:</div>
                        <div >{selectedDetails.unitcost}</div>
                      </div>
                    </>
                    }                 
                </div>
